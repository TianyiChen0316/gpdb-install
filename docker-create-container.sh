...
docker exec -it gpdb-${coordinator} /bin/bash -c 'cd ~ && source /usr/local/gpdb/greenplum_path.sh && sudo -u root /root/.startup.sh && ./auto_hosts_configure.sh && gpssh-exkeys -f gpconfigs/hostfile_exkeys && gpinitsystem -c gpconfigs/gpinitsystem_config -h gpconfigs/hostfile_gpinitsystem'
